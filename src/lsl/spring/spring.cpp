...
{
  LSL( _T("launching spring with replay: ") + filename );
...
bool Spring::Run( SinglePlayerBattle& battle )
{
  ...
  try
  {
    ...
    if ( !wxFile::Access( path, wxFile::write ) )
    {
      wxLogError( _T("Access denied to script.txt at %s"), path.c_str() );
    }
...
bool Spring::RunScript(const wxString &script)
{
    wxString path = sett().GetCurrentUsedDataDir() + wxFileName::GetPathSeparator() + _T("script.txt");
...
bool Spring::LaunchSpring( const wxString& params  )
{
  if ( m_running )
  {
    ...
    #else
        qDebug() << "spring binary not found at set location: " << ToQString( sett().GetCurrentUsedSpringBinary() );
    #endif
        return false;
    }
