...
bool IBattle::IsSynced()
{
    ...
    LoadMap();
    bool synced = true;
    if ( !m_host_map.hash.IsEmpty() || m_host_map.hash != _T("0") ) synced = synced && (m_local_map.hash == m_host_map.hash);
    if ( !m_host_map.name.IsEmpty() ) synced = synced && (m_local_map.name == m_host_map.name);
    if ( !m_host_mod.hash.IsEmpty() || m_host_mod.hash != _T("0") ) synced = synced && (m_local_mod.hash == m_host_mod.hash);
